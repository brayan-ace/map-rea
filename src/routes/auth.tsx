    ...
    >
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl items-center justify-center p-3 sm:p-6 md:p-8">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, color-mix(in oklab, var(--primary) 60%, transparent), color-mix(in oklab, var(--accent) 50%, transparent), color-mix(in oklab, var(--primary) 60%, transparent))",
          }}
        >
          <div className="relative w-full overflow-hidden rounded-[26px] glass shadow-[var(--shadow-card)]">
            ...
            <div className="relative hidden md:block" style={{ height: 640 }}>
              ...
              >
                <LoginForm
                  key={mode === "login" ? "login-active" : "login-idle"}
                  active={mode === "login"}
                />
              </div>
              ...
              >
                <SignupForm
                  key={mode === "signup" ? "signup-active" : "signup-idle"}
                  active={mode === "signup"}
                />
              </div>
            </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path
          d="M 30 520 Q 120 380 200 360 T 380 120"
          fill="none"
          stroke="white"
          strokeWidth="2"
          strokeDasharray="6 6"
      ...
      <div className="relative">
        ...
        <h2
          className={`mt-6 font-display font-bold tracking-tight ${compact ? "text-2xl" : "text-4xl"}`}
        >
        ...
        {!compact && (
          <p className="mt-4 max-w-sm text-sm text-white/75">
            Scan any city, surface local businesses missing a web presence, and pitch them on
            WhatsApp in one tap.
          </p>
        )}
      </div>
      ...
      {!compact && (
        <ul className="relative space-y-3 text-sm text-white/85">
          <li className="flex items-start gap-2">
            <Search className="mt-0.5 h-4 w-4 shrink-0" /> Smart search across categories & areas
          </li>
          <li className="flex items-start gap-2">
            <MapPin className="mt-0.5 h-4 w-4 shrink-0" /> 100km radius, deduped, ranked leads
          </li>
          <li className="flex items-start gap-2">
            <MessageCircle className="mt-0.5 h-4 w-4 shrink-0" /> One-tap personalized WhatsApp
            outreach
          </li>
        </ul>
      )}
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        >
          <div className="relative">
            <Input
              ...
              className="h-11 pr-10 focus-glow"
            />
            <button
              type="button"
        ...
        <div className="animate-rise" style={{ animationDelay: "100ms" }}>
          <Button
            ...
            className="h-11 w-full text-sm font-semibold transition-transform hover:scale-[1.01]"
            style={{ background: "var(--gradient-primary)" }}
          >
...
function SignupForm({ active }: { active: boolean }) {
  ...
  const handleSignup = async (e: FormEvent) => {
    ...
    try {
      ...
      await upsertUserDoc(
        res.user.uid,
        { uid: res.user.uid, name, email, provider: "email" },
        true,
      );
  ...
  return (
    <FormShell
      ...
      subtitle="Start finding website-less businesses today."
      active={active}
    >
    ...
    >
      {busy ? (
        ...
        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
          ...
          />
          <path
            d="M9 18c2.43 0 4.4673-.806 5.9564-2.1805l-2.9087-2.2581c-.806.54-1.8368.8595-3.0477.8595-2.344 0-4.3282-1.5832-5.0359-3.7104H.9573v2.3318C2.4382 15.9832 5.4818 18 9 18z"
            fill="#34A853"
          />
          <path
    ...
    <div className="mt-2 space-y-1">
      <div className="flex gap-1">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className={`h-1 flex-1 rounded-full transition-colors ${i <= strength ? colors[strength] : "bg-muted"}`}
          />
        ))}
      </div>
      {strength > 0 && (
        <p className="text-[11px] text-muted-foreground">Password strength: {labels[strength]}</p>
      )}
    </div>
  );
}
