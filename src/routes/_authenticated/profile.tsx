...
function ProfilePage() {
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      const template: WhatsAppTemplate = {
        ...
        content: editContent,
        isDefault: editingId
          ? (templates.find((t) => t.id === editingId)?.isDefault ?? false)
          : templates.length === 0,
        createdAt: editingId
          ? (templates.find((t) => t.id === editingId)?.createdAt ?? Date.now())
          : Date.now(),
        updatedAt: Date.now(),
      };
  ...
  return (
    <div className="px-5 sm:px-8 lg:px-12 py-8 lg:py-12 max-w-[1100px] mx-auto pb-24">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-muted-foreground truncate flex items-center gap-2 mt-1 flex-wrap">
              ...
              {user.emailVerified ? (
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] uppercase tracking-wider border-amber-500/40 text-amber-500"
                >
                  Unverified
                </Badge>
              )}
      ...
      {/* Profile */}
      <div
        className="rounded-3xl mt-5 p-6 card-premium animate-rise"
        style={{ animationDelay: "60ms" }}
      >
        <SectionHeader
          icon={<UserIcon className="w-4 h-4" />}
          title="Profile"
          sub="How you appear in WhatsApp messages."
        />
          ...
          <Field label="Phone (optional)">
            <div className="relative">
              ...
              <Input
                ...
                className="h-11 pl-9 bg-background/40 border-border/60"
              />
            </div>
          </Field>
      ...
      >
        <SectionHeader
          ...
          title="Appearance"
          sub="Choose how Lead Finder looks."
        />
      ...
      >
        <SectionHeader
          ...
          title="Security"
          sub="Manage password and email verification."
        />
        <div className="mt-5 grid sm:grid-cols-2 gap-3">
          <Button
            variant="outline"
            onClick={handlePasswordReset}
            className="h-11 bg-background/40 border-border/60 justify-start"
          >
        ...
        {(resetMsg || verifyMsg) && (
          <p className="mt-3 text-sm text-emerald-400 inline-flex items-center gap-1.5">
            <CheckCircle2 className="w-4 h-4" />
            {resetMsg || verifyMsg}
          </p>
        )}
      ...
      >
        <SectionHeader
          ...
          title="Data"
          sub="Manage your local cache."
        />
        <div className="mt-5">
          <Button
            variant="outline"
            onClick={clearLocalData}
            className="h-11 bg-background/40 border-border/60 text-destructive hover:text-destructive"
          >
            <Trash2 className="w-4 h-4 mr-2" />
            Clear local saved searches
          </Button>
        </div>
      </div>
...
function SectionHeader({
  ...
  return (
    <div className="flex items-start gap-3">
      <div
        className="w-9 h-9 rounded-xl grid place-items-center text-primary-foreground shrink-0"
        style={{ background: "var(--gradient-primary)" }}
      >
        {icon}
      </div>
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="w-8 h-8 rounded-lg grid place-items-center bg-background/60 text-foreground">
            {icon}
          </span>
